...
namespace tair
{
  namespace common
  {
      ...
      else
      {
        ...
        else
        {
          ...
          log_warn("init ring buffer record logger success, base_: %lx, w_offset: %"PRI64_PREFIX"d, r_offset: %"PRI64_PREFIX"d, reverse: %d",
                   (long unsigned int)base_, *w_offset_, *r_offset_, *reverse_);
        }
      }
                                           ...
                                           data_entry* key, data_entry* value)
    {
      ...
      {
        log_debug("@@ %lx pos: %lx %d %ld %ld", (long unsigned int)base_, (long unsigned int)mem_pos, total_size, *w_offset_, *r_offset_);
