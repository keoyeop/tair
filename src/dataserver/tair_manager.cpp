...
namespace tair {
   ...
   int tair_manager::get_position(int32_t area, data_entry &key, data_entry &end_key, data_entry &value,leveldb::Iterator* &iter,int expire_time)
   {
	   if (status != STATUS_CAN_WORK) {
		   return TAIR_RETURN_SERVER_CAN_NOT_WORK;
	   }
	   ...
	   int bucket_number = get_bucket_number(key);
	   log_debug("get request will server in bucket: %d", bucket_number);
	   PROFILER_BEGIN("get_target_kv from storage engine");
	   //int rc = storage_mgr->get(bucket_number, mkey, value);
	   int rc = storage_mgr->get_target_kv(area, bucket_number, key, end_key, value, iter, expire_time);
	   PROFILER_END();
   ...
   {
	   int hash_len = key.get_prefix_size() != 0? key.get_prefix_size() : key.get_size();
	   uint32_t hashcode = tair::util::string_util::mur_mur_hash(key.get_data(), hash_len);
