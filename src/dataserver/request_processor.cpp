...
namespace tair {
	...
	int request_processor::process(request_get_range *request, bool &send_return)
	{
		...
		//first, key points to start
		key = &request->key_start;
		//get all key value
		log_info("start iterate all data");

		while(count < limit) {
			PROFILER_BEGIN("do get_position");
			rc = tair_mgr->get_position(request->area, *key, request->key_end, *data, iter, 0);
			PROFILER_END();
			if (rc == TAIR_RETURN_END_ERROR) {
				rc = TAIR_RETURN_SUCCESS;
				break;
			}

			if (rc != TAIR_RETURN_SUCCESS ) {
				//fail, will not tell the client the server failed
				log_error("get_position failed, rev:%d, key:%s", rc, key->get_data()+2);
				break;
			}

			//judge if the last
			if(*key > request->key_end)
			{
				break;
			}
			++count;
			resp->add_key_data(new data_entry(*key), new data_entry(*data));
		}
		if(iter != NULL){
			delete iter;
			iter = NULL;
